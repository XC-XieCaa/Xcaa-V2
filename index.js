    ...
    xcaa.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                xcaa.sendMessage(anu.id, halo, audio, {contextInfo: {mentionedJid: [num]}, mimetype: 'audio/mp4', ptt:true})
                } else if (anu.action == 'remove') {
                hallo = await fs.readFileSync('./mp3/jamet.mp3')
                xcaa.sendMessage(anu.id, hallo, audio, {contextInfo: {mentionedJid: [num]}, mimetype: 'audio/mp4', ptt:true})
                }
            }
        } catch (err) {
            console.log(err)
